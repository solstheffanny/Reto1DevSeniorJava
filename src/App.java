...
//Sol Stheffanny Ramírez Arias
//Alejandro Esteban Ramírez Méndez 
...
public class App {
    ...
    public static void seleccionDePasajeros() {
        ...
        // Solicitar al usuario la cantidad de pasajeros que viajarán, si la cantidad es
        // superior a la capacidad de la nave específica, se solicita nuevamente la
        // información o el cambio de nave
    ...
    public static void calculoDeRecursos() {
        ...
        // El factor de combustible para viajes espacial es de 0.7 por km
        ...
        // Impresión de un resumen de la información que el usuario a seleccionado
        // anteriormente
    ...
    public static void seleccionRecursos() {
        char opcion;
        do { // Obtener recursos adicionales
            System.out.println("\n************************************************");
            ...
            // Consumo minimo obtenido por la distancia, tiempo y cantidad de pasajeros,
            // para añadirlo a los recursos adicionales asignados.
            combustibleMinimo = distancia[planetaSeleccionado] * factorCombustible;
            oxigenoMinimo = totalDias * factorOxigeno * cantidadDePasajeros;
            totalOxigeno = oxigenoMinimo + oxigenoExtra;
            totalCombustible = combustibleMinimo + combustibleExtra;

            // Opción para que los usuarios determinen la distribución
            switch (opcion) {
                case 'P':
                ...
                // Distribución de recursos según sea el caso
                case 'O':
        ...
        // Impresion de los recursos asignados para el viaje
    ...
    public static void inicioViaje() {
        // Llama la función random para iniciar el viaje interplanetario
        random();
        presionaEnter();
    }
    ...
    public static void random() {
        ...
        // Crear el JFrame
        JFrame frame = new JFrame("Viaje interplanetario a " + planetas[planetaSeleccionado]);
        frame.setSize(700, 100);
        frame.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
        frame.setLayout(new BoxLayout(frame.getContentPane(), BoxLayout.Y_AXIS));

        // Crear la barra de avance
        ...
        // Escritura de los recursos en el jframe
        ...
        for (int i = 0; i <= 100; i++) {
            ...
            // Descuento en porcentaje a medida que avanza el viaje
            totalCombustible -= (reduccionCombustible);
            ...
            if (i > 10 && i < 98 && random.nextInt(10) == 0) { // Probabilidad de 1/10
                ...
                // Esperar 1 segundo antes de aplicar los cambios después de los eventos
                // aleatorios
                try {
                ...
                // Mostrar el evento y recursos actualizados en el jframe
                etiquetaEvento.setText("Evento: " + evento);
                etiquetaRecursos.setText(
                        "Combustible: " + formatNumber(totalCombustible) + " | Oxígeno: " + formatNumber(totalOxigeno));
                ...
                try {
                    Thread.sleep(5000); // Mostrar evento por 5 segundos
                } catch (InterruptedException e) {
                    e.printStackTrace();
                }

                // Verificar si los recursos se han agotado tras el evento // imprimir en la
                // consola y el jframe
                if (totalCombustible <= 0 || totalOxigeno <= 0) {
                    etiquetaEvento.setText("¡Has perdido! Recursos agotados.");
                    etiquetaRecursos.setText("Combustible: " + Math.max(totalCombustible, 0) +
                ...
                // Mensaje después del evento del progreso del viaje
                etiquetaEvento.setText("Continuando el viaje...");
            }

            try {
                Thread.sleep(600); // Pausa en cada 1% de avance para simular el viaje
            } catch (InterruptedException e) {
                e.printStackTrace();
            }
        }

        // Finalizar el viaje si no se han agotado los recursos // imprime resultados
        // tanto en el jframe como en la consola
        if (totalCombustible > 0 && totalOxigeno > 0) {
        ...
        try {
            Thread.sleep(5000); // Esperar 5 segundos antes de cerrar
        } catch (InterruptedException e) {
            e.printStackTrace();
        }
        frame.dispose(); // llamado para cerrar la ventana del jframe automáticamente
    }

    public static Object[] eventoAleatorio() { // Creacion de los eventos aleatorios
        String[] eventos = {
        ...
        double[] cambiosCombustible = { -0.1, -0.05, 0, 0, +0.05, 0.05, -0.15, +0.15, +0.1, 0, +0.20, +0.1 }; // Determinar
                                                                                                              // cuanto
                                                                                                              // combustible
                                                                                                              // disminuye
                                                                                                              // o
                                                                                                              ...
                                                                                                              // evento
                                                                                                              // aleatorio
        double[] cambiosOxigeno = { 0, -0.05, -0.15, -0.1, +0.05, +0.05, -0.15, +0.15, 0, -0.08, 0, +0.1 }; // Determinar
                                                                                                            // cuanto
                                                                                                            // oxigeno
                                                                                                            // disminuye
                                                                                                            // o aumenta
        ...
        // Cambio del combustible correspondiente teniendo en cuenta el porcentaje de
        // variación en cada evento
        double cambioCombustible = cambiosCombustible[indiceEvento] * totalCombustible;
        double cambioOxigeno = cambiosOxigeno[indiceEvento] * totalOxigeno;

        // Aplicar el cambio de recursos por el evento mientras el recorrido está en
        // curso
    ...
    public static void reinicio() {
        // Reiniciar todas las opciones y volver a empezar
    ...
    public static String formatNumber(double number) {
        NumberFormat formatter = NumberFormat.getNumberInstance(Locale.US); // Opcion de separador de miles
        formatter.setGroupingUsed(true); // Activar agrupación por miles
        formatter.setMaximumFractionDigits(2); // Reducir a 2 decimales
        formatter.setMinimumFractionDigits(0); // No mostrar decimales innecesarios como ".00"
        return formatter.format(number);
    }

    public static void presionaEnter() { // Creado para que el usuario pueda leer toda la información antes de continuar
